...
public final class DownloadInfo {
    ...
    public static class Reader {
        ...
        /**
         * generate a new DownloadInfo from the database, suppose the cusor has move the the correct positon.
         ...
         * @param info the info need to be updated
         */
        public void updateFromDatabase(DownloadInfo info) {
            synchronized (info) { // add by caohaitao fix SEARHBOX-65 【百度搜索】【Android一期】【下载】下载历史记录中，文件大小不显示
                ...
                info.mRetryAfter = retryRedirect & 0xfffffff; // SUPPRESS CHECKSTYLE
                info.mRedirectCount = retryRedirect >> 28; // SUPPRESS CHECKSTYLE
    ...
    public long restartTime(long now) {
        ...
        // 修改等待策略，之前策略时间太长。
        //return mLastMod + Constants.RETRY_FIRST_DELAY * (1000 + mFuzz) * (1 << (mNumFailed - 1)); // SUPPRESS CHECKSTYLE
        return mLastMod + Constants.RETRY_FIRST_DELAY * 1000; // SUPPRESS CHECKSTYLE
    }
    ...
    void startIfReady(long now) {
        if (!isReadyToStart(now) || DownloadService.mCurrentThreadNum > Constants.MAX_SYNC_THREAD) {
            return;
        }
        ...
        if (Constants.LOGV) {
            Log.i(Constants.TAG, "startIfReady() set status: STATUS_RUNNING");
        }
    ...
    void startDownloadThread() {
        startIfReady(System.currentTimeMillis());
    }
    
    /**
     * 更新DownloadInfo的状态，并更新到数据库
     * @param status DownloadInfo的状态
     */
    ...
    /**
     * notify to pause the download, due to the size of the file may be too large for the current Network.
     * @param isWifiRequired whether wifi is prefered for the current download.
     */
