...
public class ImageViewTouch extends ImageViewTouchBase {
    /**表示是否双击放大，再双击还原*/
    static final boolean mDoubleRevert = true;
    /**表示是否双击直接放大到最大*/
    static final boolean mDoubleZoomToMax = true;
    ...
    public class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public void onLongPress(MotionEvent e) {
            if (isLongClickable()) {
                if (null != mScaleDetector && !mScaleDetector.isInProgress()) {
                    setPressed(true);
                    performLongClick();
                }
            }
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if (mScaleEnabled) {
                if (mScaled && span != 0) {
                    ...
                    // targetScale = Math.min( getMaxScale(), Math.max( targetScale, getMinScale() -
                    // 0.1f ) );
                    targetScale = Math.min(getMaxScale(), Math.max(targetScale, getMinScale() - 0.0f));
