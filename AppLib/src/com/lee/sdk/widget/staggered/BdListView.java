 ...
 * </p>
 *  Some terminology:
 *     <li>index    - index of the items that are currently visible
 *     <li>position - index of the items in the cursor
 * </p></br>
 * 
...
public class BdListView extends BdAbsListView {
    ...
    private ArrayList<FixedViewInfo> mHeaderViewInfos = new ArrayList<BdListView.FixedViewInfo>();
    private ArrayList<FixedViewInfo> mFooterViewInfos = new ArrayList<BdListView.FixedViewInfo>();
    ...
    public boolean removeHeaderView(View v) {
        if (mHeaderViewInfos.size() > 0) {
            ...
            if (((BdHeaderViewListAdapter) mAdapter).removeHeader(v)) {
    ...
    public boolean removeFooterView(View v) {
        if (mFooterViewInfos.size() > 0) {
            boolean result = false;
            if (((BdHeaderViewListAdapter) mAdapter).removeFooter(v)) {
                mDataSetObserver.onChanged();
                result = true;
            }
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
            mAdapter = new BdHeaderViewListAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
        } else {
            mAdapter = adapter;
        }
    ...
    protected void fillGap(boolean down) {
        ...
        if (down) {
            final int startOffset = count > 0 ? mDividerHeight : 0;
            fillDown(mFirstPosition + count, getItemTop(mFirstPosition + count) + startOffset);
            onAdjustChildViews(down);
        } else {
            final int startOffset = count > 0 ? -mDividerHeight : 0;
            fillUp(mFirstPosition - 1, getItemBottom(mFirstPosition - 1) + startOffset);
            onAdjustChildViews(down);
        }
    }
    ...
    private View fillDown(int pos, int top) {
        ...
        while (childTop < end && pos < mItemCount) {
            ...
            makeAndAddView(pos, getItemTop(pos) + mDividerHeight, true, false);
            pos++;
     ...
     * 
     * @param pos The first position to put in the list
     * @param nextBottom The location where the bottom of the item associated with pos should be
     *            drawn
     * 
    ...
    private View fillUp(int pos, int bottom) {
        ...
        while (childBottom > end && pos >= 0) {
            // is this the selected item?
            makeAndAddView(pos, getItemBottom(pos) - mDividerHeight, false, false);
            // nextBottom = child.getTop() - mDividerHeight;
            pos--;
            
            // 解决一个BUG，快速向上滑动时，有时候可能导致界面变成空白，原因是
            // 我们在填充一个child后，由于派生类可能是多列，所以我们应该找出所有列
            // 中最下面的位置，而getItemBottom(pos)是找出当前列的最后位置。
            //childBottom = getItemBottom(pos);
            childBottom = getFillChildTop() - mDividerHeight;
        }
    ...
    final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition, final int maxHeight,
        ...
        final BdAbsListView.RecycleBin recycleBin = mRecycler;
    ...
    private void setupChild(View child, int position, int y, boolean flowDown, int childrenLeft, boolean selected, boolean recycled) {
        ...
        BdAbsListView.LayoutParams p = (BdAbsListView.LayoutParams) child.getLayoutParams();
        if (p == null) {
            p = new BdAbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0);
        }
        p.viewType = mAdapter.getItemViewType(position);

        if ((recycled && !p.forceAdd) || (p.recycledHeaderFooter && p.viewType == BdAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER)) {
            attachViewToParent(child, flowDown ? -1 : 0, p);
        } else {
            p.forceAdd = false;
            if (p.viewType == BdAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER) {
    ...
    private void scrollListItemsBy(int amount) {
        ...
        if (amount < 0) {
            ...
            while (first.getBottom() < listTop) {
                BdAbsListView.LayoutParams layoutParams = (LayoutParams) first.getLayoutParams();
            ...
            while (last.getTop() > listBottom) {
                BdAbsListView.LayoutParams layoutParams = (LayoutParams) last.getLayoutParams();
                if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
