...
package com.lee.sdk.widget.dragsort;
...
public class DragSortListView extends ListView {
     ...
     * the dragged item.
     * 悬浮框位置计算在dispatchDraw中，左上角坐标和被拖拽view左上角相同
     */
    ...
    /**
     * 被拖拽的itemview在listview中的正中间坐标.
     */
    private int mDragViewMid;
     ...
     */
    private int mItemHeightCollapsed = DEFAULT_ITEM_HEIGHT_COLLAPSED;
    /**
     * 默认被拖拽view的折叠高度，即拖拽过后原被选中位置最小高度
     */
    public final static int DEFAULT_ITEM_HEIGHT_COLLAPSED = 0;

    /**
     * 被拖拽view在listview中的高度，即拖拽的时候listview中的invisible itemview高度
     * mDragViewMid、mDragViewHeight、mDragViewHeightHalf用于计算listview中待拖拽最终位置的空白区域
     */
     ...
     */
    private int mDragViewHeightHalf;
    ...
    /** 浮动图背景是否透明*/
    private boolean mIsFloatViewTransparency;
    /** 浮动图背景透明度,默认值不透明*/
    private int mFloatViewAlpha = 255;
    /** 是否绘制分割线*/
    ...
    public DragSortListView(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            mItemHeightCollapsed = Math.max(DEFAULT_ITEM_HEIGHT_COLLAPSED, a.getDimensionPixelSize(
                    R.styleable.DragSortListView_collapsed_height, DEFAULT_ITEM_HEIGHT_COLLAPSED));
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mFloatView != null) {
            ...
            if (mIsFloatViewTransparency) {
                ...
                mFloatViewAlpha = (int) (255f * mCurrFloatAlpha * alphaMod);
            }

            canvas.save();
            // mFloatLoc.y是悬浮框绘制的左上角Y坐标位置，该值和被选中的itemview左上角Y坐标位置相同
            // 当悬浮框的大小和Listview的itemview大小不一样，为了使悬浮框悬浮时正中和itemview正中重合，
            // 需要调整偏移，框中Listview的拖拽空白为itemview高度，即mDragViewHeight，
            // itemview正中位置mFloatLoc.y + mDragViewHeight / 2，则悬浮框绘制左上角Y坐标位置为正中位置减去悬浮框高度一半
            canvas.translate(mFloatLoc.x, mFloatLoc.y + mDragViewHeight / 2 - mFloatView.getMeasuredHeight() / 2);
            canvas.clipRect(0, 0, w, h);

            // Log.d("mobeta", "clip rect bounds: " + canvas.getClipBounds());
            canvas.saveLayerAlpha(0, 0, w, h, mFloatViewAlpha, Canvas.ALL_SAVE_FLAG);
    ...
    private int getShuffleEdge(int position, int top) {
        ...
        int maxBlankHeight = mDragViewHeight - mItemHeightCollapsed;
        ...
        if (mSecondExpPos <= mSrcPos) {
            ...
            if (position == mSecondExpPos && mFirstExpPos != mSecondExpPos) {
                if (position == mSrcPos) {
                    otop = top + itemHeight - mDragViewHeight;
                } else {
        ...
        // otop is set
        if (position <= mSrcPos) {
            edge = otop + (mDragViewHeight - divHeight - getChildHeight(position - 1)) / 2;
        } else {
            edge = otop + (childHeight - divHeight - mDragViewHeight) / 2;
        }
    ...
    private boolean updatePositions() {
        ...
        if (mAnimate) {
            ...
            // Three regions
            if (mDragViewMid < slideEdgeTop) {
                mFirstExpPos = itemPos - 1;
                mSecondExpPos = itemPos;
                mSlideFrac = 0.5f * ((float) (slideEdgeTop - mDragViewMid)) / slideRgnHeightF;
                // Log.d("mobeta",
                // "firstExp="+mFirstExpPos+" secExp="+mSecondExpPos+" slideFrac="+mSlideFrac);
            } else if (mDragViewMid < slideEdgeBottom) {
                mFirstExpPos = itemPos;
                mSecondExpPos = itemPos;
            } else {
                mFirstExpPos = itemPos;
                mSecondExpPos = itemPos + 1;
                mSlideFrac = 0.5f * (1.0f + ((float) (edgeBottom - mDragViewMid))
                        / slideRgnHeightF);
                // Log.d("mobeta",
                // "firstExp="+mFirstExpPos+" secExp="+mSecondExpPos+" slideFrac="+mSlideFrac);
            }
        ...
        if (mFirstExpPos != oldFirstExpPos || mSecondExpPos != oldSecondExpPos
                || 0 != Float.compare(mSlideFrac, oldSlideFrac)) {
            updated = true;
        }
    ...
    private class LiftAnimator extends SmoothAnimator {
        ...
        public void onStart() {
            ...
            mFinalDragDeltaY = mDragViewHeightHalf;
        }
        ...
        private int getTargetY() {
            ...
            if (v != null) {
                if (mDropPos == srcPos) {
                    ...
                    // expanded up
                    targetY = v.getBottom() + otherAdjust - mDragViewHeight;
                }
            } else {
    ...
    private void continueDrag(int x, int y) {
        ...
        int minY = Math.min(y, mDragViewMid + mDragViewHeightHalf);
        int maxY = Math.max(y, mDragViewMid - mDragViewHeightHalf);
    ...
    private int calcItemHeight(int position, int childHeight) {
        ...
        int maxNonSrcBlankHeight = mDragViewHeight - mItemHeightCollapsed;
    ...
    private void measureFloatView() {
        ...
        // 为了保持浮动图的大小不影响Listview底部的invisible item大小,mFloatViewHeight设置成被拖拽item高度
        View view = getChildAt(mSrcPos - getFirstVisiblePosition());
        if (view != null) {
            measureItem(view);
            mDragViewHeight = view.getMeasuredHeight();
            mDragViewHeightHalf = mDragViewHeight / 2;
        }
    }
    ...
    private void updateFloatView() {
        ...
        } else if (floatY + mDragViewHeight > bottomLimit) {
            mFloatLoc.y = bottomLimit - mDragViewHeight;
        }

        // get y-midpoint of floating view (constrained to ListView bounds)
        mDragViewMid = mFloatLoc.y + mDragViewHeightHalf;
    }
    ...
    private class DragScroller implements Runnable {
        ...
        public void run() {
            ...
            int minY = Math.min(mY, mDragViewMid + mDragViewHeightHalf);
            int maxY = Math.max(mY, mDragViewMid - mDragViewHeightHalf);
     ...
     */
    public void setFloatViewTransparency(boolean isTransparency){
        mIsFloatViewTransparency = isTransparency;
    }
    
    /**
     * 设置是否绘制分割线
     * 
     * @param isTransparency 是否绘制分割线
     */
    public void setDrawDivider(boolean isDrawDivider){
        mDrawDivider = isDrawDivider;
    }
    ...
    private class DragSortTracker {
        ...
        public DragSortTracker() {
            ...
            if (!mFile.exists()) {
                try {
                    ...
                    if (DEBUG) {
                        Log.w(TAG, "Could not create dslv_state.txt");
        ...
        public void appendState() {
            ...
            mBuilder.append("    <SrcHeight>").append(mDragViewHeight + getDividerHeight())
                    .append("</SrcHeight>\n");
            mBuilder.append("    <ViewHeight>").append(getHeight()).append("</ViewHeight>\n");
            mBuilder.append("    <LastY>").append(mLastY).append("</LastY>\n");
            mBuilder.append("    <FloatY>").append(mDragViewMid).append("</FloatY>\n");
