...
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
    ...
    /**
     * 获取当前被点击位置
     * @author shengshuqiang
     *
     * @return 当前被点击位置
     */
    protected int getClickRemoveHitPos() {
        return mClickRemoveHitPos;
    }
     ...
     * 更新操作结束后的操作状态
     */
    private void updateEndOperateStatus() {
        // 标记当前状态为非移动删除状态
        mIsRemoving = false;
        // 标记当前状态为非拖拽状态
        mDragging = false;
    ...
    public boolean onDown(MotionEvent ev) {
        if (mRemoveEnabled && mRemoveMode == CLICK_REMOVE) {
            ...
            // 重置
            mClickRemoveHitPos = MISS;
        }

        mHitPos = startDragPosition(ev);
        // 如果拖拽和删除同时触发，只处理删除
        if (mClickRemoveHitPos == MISS && mHitPos != MISS && mDragInitMode == ON_DOWN) {
