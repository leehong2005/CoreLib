...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    /**拉动最大的距离，如果<0表示不限制拉动距离*/
    private int mMaxPullOffset = -1;
    ...
    @Override
    public void setMaxPullOffset(int maxOffset) {
        mMaxPullOffset = maxOffset;
    }
    ...
    protected void pullHeaderLayout(float delta) {
        ...
        // 如果滑动距离超过最大值，不滑动
        if (mMaxPullOffset > 0) {
            if (delta > 0 && Math.abs(oldScrollY) >= mMaxPullOffset) {
                return;
            }
        }
