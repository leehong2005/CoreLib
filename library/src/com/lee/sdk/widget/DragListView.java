...
/**
 * This class implements the function that user can swap position of items by
 * dragging.
 * 
 * @author Li Hong
 * 
 * @date 2013/03/06
 */
public class DragListView extends ListView implements IDropTarget {
    /**
    ...
    public interface IDragAdapter {
         ...
         * 
         * @param dragPosition
         *            The drag source position.
         * @param curPosition
         *            The current position in which drag view locates.
         * 
         *            NOTE: The subclass typically implements this method like
         *            this:
         * 
         ...
         * 
         * @param position
         *            The position of item which view should be invisible.
         * 
         *            NOTE: You need to remember the hide position passed by
         *            this method, in the
         *            {@link ListAdapter#getView(int, View, android.view.ViewGroup)}
         *            method, you typically implement like this:
         * 
        ...
        /**
         * Update the dataset, typically you call
         * {@link BaseAdapter#notifyDataChanged()} method.
         */
        public void update();
    }

    /**
     * This interface defines some extra methods, such as insert or remove item.
     * A listview or gridview can receive data from other source in other view
     * group.
     */
    public interface IDragAdapter2 extends IDragAdapter {
         ...
         * 
         *            NOTE: In this method, you should always check the data
         *            type whether matches what you expect.
         */
    ...
    /**
     * The current drag position. This member is like {@link mSrcDragPosition},
     * it will change to this value which current dragging over position when it
     * swaps items.
     */
    private int mDragPosition = INVALID_POSITION;

    /**
     * The original drag position. Please think about this case: Drag the
     * position 0 item and quickly move to position 2. In fact, we will do swap
     * animation twice.
     * 
     * <pre class="prettyprint">
    ...
    /**
     * The drag layout, we use it to start the dragging operation if we need
     * start dragging by myself.
     */
     ...
     */
    private ListViewDragListener mDragListener = null;
    ...
    public void setDragLayout(IDragLayout dragLayout) {
        ...
        this.mDragListener = new ListViewDragListener(this);
    ...
    /**
     * Set the current drag position. Typically this method can be called by the
     * drag source.
     * 
     * @param dragPosition
     ...
     * 
     * @param view
     *            This view will be dragged.
     * @param position
     *            The position of this view in its adapter.
     * 
     ...
     * 
     *      NOTE: Here return true, in fact, should check the data contained in
     *      the params whether matches what we expect. Subclass can override
     *      this method.
     */
    @Override
    ...
    /**
     * Swap the position of data in adapter. Typically this method is called
     * when all children finish doing animation.
     */
    ...
    private void doAnimation(int dragPosition, int curPosition) {
        if (dragPosition < curPosition) {
            for (int index = dragPosition + 1; index <= curPosition; index++) {
    ...
    private void insertDummyItem() {
        /*
         * if (INVALID_POSITION == mDragPosition) { if (mCurPosition !=
         * mDragPosition) { if (mDragAdapter instanceof IDragAdapter2) { int
         * lastPosition = getCount() - 1;
         * ((IDragAdapter2)mDragAdapter).insert(lastPosition,
         * mDragParams.getDataObject()); setDragPosition(lastPosition);
         * mHasInsertDummy = true; } } }
         */
    }
    ...
    /**
     * The drag listener for the grid view to scroll up and down when dragging
     * over the bottom or top rectangle.
     */
    private static class ListViewDragListener extends DragListener {
        /**
         * Scroll mode.
         */
        ...
        private Handler mHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (SCROLL_UP == mScrollMode) {
                    mListView.smoothScrollBy(mListView.getScrollDistance(), SCROLL_DURATION);
         ...
         * @param listView
         */
        public ListViewDragListener(DragListView listView) {
            mListView = listView;
        }
        ...
        public void onDragEnd() {
            ...
            if (null != mListView) {
                mListView.removeDummyItem();
                mListView.updateGridView();
                mListView.setHasDragEnd(true);
            }
        }
